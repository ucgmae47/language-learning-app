...
const LEVEL_GUIDANCE: Record<CefrLevel, string> = {
  ...
  A2: "Use everyday vocabulary. Present and simple past (pretérito indefinido). Sentences up to 12 words. 200–280 words total.",
  B1: "Use a broad everyday vocabulary including some idiomatic expressions. Mix present, past (indefinido and imperfecto), and future tenses naturally. 300–400 words total.",
...
export function buildStoryPrompt(
  ...
  if (selectedTopic) {
    const extras = userInterests.length > 0 ? ` Subtly incorporate the learner's interests (${userInterests.join(", ")}) where it fits naturally.` : "";
    themeInstruction = `Genre: **${selectedTopic}**.${extras}`;
  } else {
    const topicList = userInterests.length > 0 ? userInterests.join(", ") : DEFAULT_TOPICS.join(", ");
  ...
  return `You are an expert Spanish language educator creating immersive reading material.

TASK
Write an original short story in Spanish and then produce a 5-question comprehension quiz in English.

STORY REQUIREMENTS
- Language: entirely in Spanish — no English in the story body
...
- Title: in Spanish, evocative, 3–8 words
